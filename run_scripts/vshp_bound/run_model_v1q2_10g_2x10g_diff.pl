...
for (my $i=64; $i<=1536; $i+=64) {
    ...
    my $cmd = "./tm_osched_model.out -f knobs/v1q2.knobs +*vlan0_vshaper_rate=10 " . $port0_cfg . $port1_cfg . " >& run_$i.log";
...
my $cmd = "gnuplot -e \"set key right center; plot 'run_results.csv' using 1:2 with linespoints title 'q0_shaper: 10G', '' using 1:3 with linespoints title 'q1_shaper: 10G', '' using 1:4 with linespoints title 'vlan_shaper: 20G'; set term png; set output 'v1q2_10g_2x10g_diff.png'; replot; set term x11\"";
